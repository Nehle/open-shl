...
function buildQuerystring(queryObject) {
    return Object.keys(queryObject)
        .filter(k => typeof k === "string" && typeof queryObject[k] === "string")
        .map(k => encodeUriComponent(k) + "=" + encodeUriComponent(queryObject))
        .join("&");
}
...
Connection.prototype._fetch = function (path, query) {
    if(!this.accessToken) {
